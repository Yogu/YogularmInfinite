...
public class Component extends FormalComponent {
	...
	public boolean canMoveTo(Vector targetPosition) {
		...
		for (Component component : getCollection().getComponentsAround(path)) {
			if ((component != this) && !component.isToRemove() && (component instanceof Component) && ((Component) component).isSolid()) {
	...
	private Vector getImpactOnMove(Vector targetPosition, boolean calledOnMove, Axis axis) {
		...
		for (Component component : getCollection().getComponentsAround(path)) {
			if ((component instanceof Component) && !component.isToRemove() && (component != this)) {
				Component body = (Component) component;
				Rect obstacle = body.getOuterBounds();
				// Ignore Bodies already colliding with this body (otherwise this body
				// would be stuck)
				...
				if (!obstacle.overlaps(source)) {
					...
					if (path.overlaps(obstacle)) {
						...
						if (calledOnMove && collided && isSolid && body.isSolid) {
							...
							for (Component b : new Component[] { this, body }) {
