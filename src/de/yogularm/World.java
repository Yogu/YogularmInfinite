...
public class World {
	...
	static {
		/*builders.add(new Sky());
		builders.add(new Sky2());
		builders.add(new Farm());*/
	}
	...
	public void update(float elapsedTime) {
		...
		float actionDistance = Math.max(camera.getBounds().getWidth(), camera.getBounds().getHeight());
		
		Iterator<Component> iterator = components.iterator();
		while (iterator.hasNext()) {
			Component component = iterator.next();
			float distance = Vector.getDistance(component.getPosition(), player.getPosition());
			if (!component.isToRemove() && distance <= actionDistance)
				component.update(elapsedTime);
			if (distance > MAX_BUFFER_LENGTH
				|| component.isToRemove())
				iterator.remove();
		}
	...
	public boolean hasBlockBelow(Vector position) {
		...
		for (Component component : components) {
			if (component instanceof Block) {
				...
				if ((Math.round(component.getPosition().getX()) == rounded.getX()) &&
